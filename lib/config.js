...
exports.look_up_member_following = (mb_id, origin) => {
  ...
  return {
    //$subscriber_id:bu matchda ajratib olingan
    //$_id: bu ham matchda ajratib olingan
    $lookup: {
      from: "follows",
      let: {
      ...
      pipeline: [
        {
          $match: {
            $expr: {
              $and: [
                { $eq: ["$follow_id", "$$lc_follow_id"] }, //folow_idlari mani subscriber_idlarimga teng bo'lganlarini
                // tanla.Ikkinchi holatda, follow_idisi an tanlagan member_idga teng bo'lganini top, va
                { $eq: ["$subscriber_id", "$$lc_subscriber_id"] }, // hamda ularni subscriber_idlari mani _idimga teng bo'lganlarini tanla.
                //Ikkinchi holatda, uni subscriber_idisi mani mb_idimga teng bo'lsa tanla
              ],
            },
          },
        },
...
exports.look_up_member_liked = (mb_id) => {
  return {
    $lookup: {
      from: "likes",
      let: {
        lc_item_id: "$_id", // matchdan kelgan
        lc_mb_id: mb_id,
        nw_my_favorite: true,
      },
      pipeline: [
